...
func main() {
	cfg := pkgconfig.Load()
	if err := pkgruntimecfg.ValidateServiceConfig("kms-workload-identity", cfg); err != nil {
	...
	auditURL := envOr("AUDIT_URL", "http://audit:8070")
	...
	grpcPort := envOr("GRPC_PORT", "18250")
	...
	if reg, err := pkgconsul.NewRegistrar(cfg.ConsulAddress, "kms-workload-identity-"+httpPort, "kms-workload-identity", "127.0.0.1", mustAtoi(grpcPort)); err == nil {
...
func initNATS(url string) (*nats.Conn, nats.JetStreamContext, error) {
	nc, err := pkgevents.Connect(url, "kms-workload-identity", logger.Printf)
	if err != nil {
		return nil, nil, err
	}
	js, err := nc.JetStream()
	...
	_, _ = js.AddStream(&nats.StreamConfig{Name: "AUDIT_WORKLOAD", Subjects: []string{"audit.workload.*"}})
	return nc, js, nil
}

func migrationPath() string {
	candidates := []string{
		filepath.Join("services", "workload", "migrations"),
	...
	return filepath.Join("services", "workload", "migrations")
}
...
func devMTLSConfig() (*tls.Config, error) {
	...
	tpl := &x509.Certificate{
		SerialNumber:          serial,
		Subject:               pkix.Name{CommonName: "kms-workload-identity-local"},
