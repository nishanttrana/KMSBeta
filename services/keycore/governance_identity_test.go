...
func TestGovernanceCallsCarryServiceIdentity(t *testing.T) {
	auth := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		_ = json.NewEncoder(w).Encode(map[string]string{"access_token": "kms-keycore-jwt", "expires_at": "2099-01-01T00:00:00Z"})
	}))
	defer auth.Close()
	...
	if _, err := NewHTTPFIPSModeProvider(gov.URL, 0, 0).IsEnabled(context.Background(), "tenant-a"); err != nil {
		t.Fatal(err)
	}
	if _, err := NewHTTPGovernancePostureControlsProvider(gov.URL, 0, 0).Controls(context.Background(), "tenant-a"); err != nil {
		t.Fatal(err)
	}
	mu.Lock()
	defer mu.Unlock()
	if len(seen) != 2 {
		t.Fatalf("governance calls = %d, want 2", len(seen))
	}
	for _, r := range seen {
		if r.URL.Query().Get("tenant_id") != "root" || r.Header.Get("Authorization") != "Bearer kms-keycore-jwt" {
			t.Fatalf("%s %s: tenant_id=%q Authorization=%q, want root and the kms-keycore service token",
