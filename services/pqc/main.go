...
func main() {
	cfg := pkgconfig.Load()
	logger := log.New(os.Stdout, "[kms-pqc] ", log.LstdFlags|log.LUTC)
...
func initNATS(url string) (*nats.Conn, nats.JetStreamContext, error) {
	nc, err := nats.Connect(url, nats.Name("kms-pqc"))
	if err != nil {
		return nil, nil, err
	}
	js, err := nc.JetStream()
	...
	_, _ = js.AddStream(&nats.StreamConfig{Name: "AUDIT_PQC", Subjects: []string{"audit.pqc.*"}})
	return nc, js, nil
}
...
func devMTLSConfig() (*tls.Config, error) {
	...
	tpl := &x509.Certificate{
		SerialNumber:          serial,
		Subject:               pkix.Name{CommonName: "kms-pqc-local"},
