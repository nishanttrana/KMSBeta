...
func main() {
	...
	if err := pkgruntimecfg.ValidateServiceConfig("kms-discovery", cfg); err != nil {
	...
	grpcPort := envOr("GRPC_PORT", "18100")
	...
	if reg, err := pkgconsul.NewRegistrar(cfg.ConsulAddress, "kms-discovery-"+httpPort, "kms-discovery", "127.0.0.1", mustAtoi(grpcPort)); err == nil {
...
func initNATS(url string) (*nats.Conn, nats.JetStreamContext, error) {
	nc, err := pkgevents.Connect(url, "kms-discovery", logger.Printf)
	if err != nil {
		return nil, nil, err
	}
	js, err := nc.JetStream()
	...
	_, _ = js.AddStream(&nats.StreamConfig{Name: "AUDIT_DISCOVERY", Subjects: []string{"audit.discovery.*"}})
	return nc, js, nil
}

func migrationPath() string {
	candidates := []string{filepath.Join("services", "discovery", "migrations"), filepath.Join(".", "migrations")}
	for _, c := range candidates {
		if st, err := os.Stat(c); err == nil && st.IsDir() {
			return c
		}
	}
	return filepath.Join("services", "discovery", "migrations")
}
...
func devMTLSConfig() (*tls.Config, error) {
	...
	tpl := &x509.Certificate{
		SerialNumber:          serial,
		Subject:               pkix.Name{CommonName: "kms-discovery-local"},
	...
	cp.AddCert(c)
	return &tls.Config{MinVersion: tls.VersionTLS13, Certificates: []tls.Certificate{cert}, ClientAuth: tls.RequireAndVerifyClientCert, ClientCAs: cp}, nil
}
