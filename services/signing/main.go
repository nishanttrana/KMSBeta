...
func main() {
	...
	httpSrv := pkgconfig.NewHTTPServer(httpPort, pkgauditmw.Wrap(handler, publisher, "signing"))
	go func() {
...
func initNATS(url string) (*nats.Conn, nats.JetStreamContext, error) {
	...
	_, _ = js.AddStream(&nats.StreamConfig{Name: "AUDIT_SIGNING", Subjects: []string{"audit.signing.*"}})
	return nc, js, nil
}
...
func devMTLSConfig() (*tls.Config, error) {
	...
	tpl := &x509.Certificate{
		SerialNumber:          serial,
		Subject:               pkix.Name{CommonName: "kms-signing-local"},
