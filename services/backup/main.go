...
var logger = log.New(os.Stdout, "[kms-backup] ", log.LstdFlags|log.Lmicroseconds)
...
func main() {
	...
	if err := pkgruntimecfg.ValidateServiceConfig("kms-backup", cfg); err != nil {
	...
	if reg, err := pkgconsul.NewRegistrar(cfg.ConsulAddress, "kms-backup-"+httpPort, "kms-backup", "127.0.0.1", mustAtoi(grpcPort)); err == nil {
...
func initNATS(url string) (*nats.Conn, nats.JetStreamContext, error) {
	nc, err := nats.Connect(url, nats.Name("kms-backup"))
	if err != nil {
		return nil, nil, err
	}
	js, err := nc.JetStream()
	...
	_, _ = js.AddStream(&nats.StreamConfig{Name: "AUDIT_BACKUP", Subjects: []string{"audit.backup.*"}})
	return nc, js, nil
}

func migrationPath() string {
	candidates := []string{
		filepath.Join("services", "backup", "migrations"),
	...
	return filepath.Join("services", "backup", "migrations")
}
...
func devMTLSConfig() (*tls.Config, error) {
	...
	tpl := &x509.Certificate{
		SerialNumber:          serial,
		Subject:               pkix.Name{CommonName: "kms-backup-local"},
