...
var logger = log.New(os.Stdout, "[ai] ", log.LstdFlags|log.Lmicroseconds)

func main() {
	cfg := pkgconfig.Load()

	if err := pkgruntimecfg.ValidateServiceConfig("kms-ai", cfg); err != nil {
	...
	if nc, js, err := initNATS(cfg.NATSURL); err == nil {
		defer nc.Close()
		publisher = pkgevents.NewPublisher(js, 3, "audit.ai.dead_letter")
	} else {
		logger.Printf("nats unavailable, ai event publishing disabled: %v", err)
	}

	svc := NewService(
		NewSQLStore(dbConn),
		NewHTTPKeyCoreClient(envOr("KEYCORE_URL", "http://127.0.0.1:8010"), 5*time.Second),
		NewHTTPPolicyClient(envOr("POLICY_URL", "http://127.0.0.1:8040"), 5*time.Second),
		...
		NewHTTPReportingClient(envOr("REPORTING_URL", "http://127.0.0.1:8140"), 5*time.Second),
		NewHTTPSecretsClient(envOr("SECRETS_URL", "http://127.0.0.1:8020"), 5*time.Second),
		NewHTTPLLMBackend(45*time.Second),
		publisher,
	)
	handler := NewHandler(svc)

	httpPort := envOr("HTTP_PORT", "8090")
	httpSrv := pkgconfig.NewHTTPServer(httpPort, pkgauditmw.Wrap(handler, publisher, "ai"))
	go func() {
	...
	grpcPort := envOr("GRPC_PORT", "18090")
	...
	if reg, err := pkgconsul.NewRegistrar(cfg.ConsulAddress, "kms-ai-"+httpPort, "kms-ai", "127.0.0.1", mustAtoi(grpcPort)); err == nil {
...
func initNATS(url string) (*nats.Conn, nats.JetStreamContext, error) {
	nc, err := pkgevents.Connect(url, "kms-ai", logger.Printf)
	if err != nil {
		return nil, nil, err
	}
	js, err := nc.JetStream()
	...
	_, _ = js.AddStream(&nats.StreamConfig{Name: "AUDIT_AI", Subjects: []string{"audit.ai.*"}})
	return nc, js, nil
}

func migrationPath() string {
	candidates := []string{
		filepath.Join("services", "ai", "migrations"),
	...
	return filepath.Join("services", "ai", "migrations")
}
...
func devMTLSConfig() (*tls.Config, error) {
	...
	tpl := &x509.Certificate{
		SerialNumber:          serial,
		Subject:               pkix.Name{CommonName: "kms-ai-local"},
