...
func (h *Handler) routes() *http.ServeMux {
	...
	mux.HandleFunc("GET /auth/scim/settings", h.withAuth(h.handleGetSCIMSettings, "auth.user.read"))
	mux.HandleFunc("PUT /auth/scim/settings", h.withAuth(h.handleUpdateSCIMSettings, "auth.user.write"))
	mux.HandleFunc("POST /auth/scim/settings/rotate-token", h.withAuth(h.handleRotateSCIMToken, "auth.user.write"))
	mux.HandleFunc("GET /auth/scim/summary", h.withAuth(h.handleGetSCIMSummary, "auth.user.read"))
	mux.HandleFunc("GET /auth/scim/users", h.withAuth(h.handleListSCIMUsersAdmin, "auth.user.read"))
	mux.HandleFunc("GET /auth/scim/groups", h.withAuth(h.handleListSCIMGroupsAdmin, "auth.user.read"))
	...
	mux.HandleFunc("GET /scim/v2/ResourceTypes", h.handleSCIMResourceTypes)
	mux.HandleFunc("GET /scim/v2/Users", h.handleSCIMListUsers)
	mux.HandleFunc("POST /scim/v2/Users", h.handleSCIMCreateUser)
	mux.HandleFunc("GET /scim/v2/Users/{id}", h.handleSCIMGetUser)
	mux.HandleFunc("PUT /scim/v2/Users/{id}", h.handleSCIMReplaceUser)
	mux.HandleFunc("PATCH /scim/v2/Users/{id}", h.handleSCIMPatchUser)
	mux.HandleFunc("DELETE /scim/v2/Users/{id}", h.handleSCIMDeleteUser)
	mux.HandleFunc("GET /scim/v2/Groups", h.handleSCIMListGroups)
	mux.HandleFunc("POST /scim/v2/Groups", h.handleSCIMCreateGroup)
	mux.HandleFunc("GET /scim/v2/Groups/{id}", h.handleSCIMGetGroup)
	mux.HandleFunc("PUT /scim/v2/Groups/{id}", h.handleSCIMReplaceGroup)
	mux.HandleFunc("PATCH /scim/v2/Groups/{id}", h.handleSCIMPatchGroup)
	mux.HandleFunc("DELETE /scim/v2/Groups/{id}", h.handleSCIMDeleteGroup)
	return mux
}
