...
func createSQLiteSchema(conn *pkgdb.DB) error {
	sql := []string{
		...
		`CREATE TABLE auth_client_registrations (
			...
			interface_name TEXT NOT NULL DEFAULT 'rest',
			...
			replay_protection_enabled INTEGER NOT NULL DEFAULT 1,
			mtls_cert_fingerprint TEXT,
			mtls_subject_dn TEXT,
			mtls_uri_san TEXT,
			http_signature_key_id TEXT,
			http_signature_public_key_pem TEXT,
			http_signature_algorithm TEXT,
			verified_request_count INTEGER NOT NULL DEFAULT 0,
			replay_violation_count INTEGER NOT NULL DEFAULT 0,
			signature_failure_count INTEGER NOT NULL DEFAULT 0,
			unsigned_reject_count INTEGER NOT NULL DEFAULT 0,
			last_verified_request_at TIMESTAMP,
			last_replay_violation_at TIMESTAMP,
			last_signature_failure_at TIMESTAMP,
			last_unsigned_reject_at TIMESTAMP,
			last_auth_mode_used TEXT,
			created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
			approved_at TIMESTAMP,
			expires_at TIMESTAMP,
			last_used TIMESTAMP
		);`,
		`CREATE TABLE auth_request_nonce_cache (
			tenant_id TEXT NOT NULL,
			nonce TEXT NOT NULL,
