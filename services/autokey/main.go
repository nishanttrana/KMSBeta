...
var logger = log.New(os.Stdout, "[autokey] ", log.LstdFlags|log.Lmicroseconds)

func main() {
	cfg := pkgconfig.Load()
	if err := pkgruntimecfg.ValidateServiceConfig("kms-autokey", cfg); err != nil {
	...
	if nc, js, err := initNATS(cfg.NATSURL); err == nil {
		defer nc.Close()
		publisher = pkgevents.NewPublisher(js, 3, "audit.autokey.dead_letter")
	} else {
	...
	if reg, err := pkgconsul.NewRegistrar(cfg.ConsulAddress, "kms-autokey-"+httpPort, "kms-autokey", "127.0.0.1", mustAtoi(grpcPort)); err == nil {
...
func initNATS(url string) (*nats.Conn, nats.JetStreamContext, error) {
	nc, err := pkgevents.Connect(url, "kms-autokey", logger.Printf)
	if err != nil {
		return nil, nil, err
	}
	js, err := nc.JetStream()
	...
	_, _ = js.AddStream(&nats.StreamConfig{Name: "AUDIT_AUTOKEY", Subjects: []string{"audit.autokey.*"}})
	return nc, js, nil
}

func migrationPath() string {
	candidates := []string{
		filepath.Join("services", "autokey", "migrations"),
	...
	return filepath.Join("services", "autokey", "migrations")
}
...
func devMTLSConfig() (*tls.Config, error) {
	...
	tpl := &x509.Certificate{
		SerialNumber:          serial,
		Subject:               pkix.Name{CommonName: "kms-autokey-local"},
