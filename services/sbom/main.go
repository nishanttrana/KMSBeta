...
var logger = log.New(os.Stdout, "[sbom] ", log.LstdFlags|log.Lmicroseconds)

func main() {
	cfg := pkgconfig.Load()

	if err := pkgruntimecfg.ValidateServiceConfig("kms-sbom", cfg); err != nil {
	...
	svc.StartScheduler(ctx, SchedulerConfig{
		SBOMMode: envOr("SBOM_SCHEDULE", "off"),
		CBOMMode: envOr("CBOM_SCHEDULE", "off"),
	...
	grpcPort := envOr("GRPC_PORT", "18180")
	...
	if reg, err := pkgconsul.NewRegistrar(cfg.ConsulAddress, "kms-sbom-"+httpPort, "kms-sbom", "127.0.0.1", mustAtoi(grpcPort)); err == nil {
...
func initNATS(url string) (*nats.Conn, nats.JetStreamContext, error) {
	nc, err := pkgevents.Connect(url, "kms-sbom", logger.Printf)
	if err != nil {
		return nil, nil, err
	}
	js, err := nc.JetStream()
	...
	_, _ = js.AddStream(&nats.StreamConfig{Name: "AUDIT_SBOM", Subjects: []string{"audit.sbom.*", "audit.cbom.*"}})
	return nc, js, nil
}

func migrationPath() string {
	candidates := []string{
		filepath.Join("services", "sbom", "migrations"),
	...
	return filepath.Join("services", "sbom", "migrations")
}
...
func devMTLSConfig() (*tls.Config, error) {
	...
	tpl := &x509.Certificate{
		SerialNumber:          serial,
		Subject:               pkix.Name{CommonName: "kms-sbom-local"},
