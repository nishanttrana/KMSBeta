...
func createDataProtectSchemaForTest(conn *pkgdb.DB) error {
	stmts := []string{
		...
		`CREATE TABLE data_protection_policy (
			...
			algorithm_profile_policy_json TEXT NOT NULL DEFAULT '{}',
			require_aad_for_aead BOOLEAN NOT NULL DEFAULT FALSE,
			required_aad_claims_json TEXT NOT NULL DEFAULT '[]',
			enforce_aad_tenant_binding BOOLEAN NOT NULL DEFAULT FALSE,
			allowed_aad_environments_json TEXT NOT NULL DEFAULT '[]',
			max_fields_per_operation INTEGER NOT NULL DEFAULT 64,
			max_document_bytes INTEGER NOT NULL DEFAULT 262144,
			max_app_crypto_request_bytes INTEGER NOT NULL DEFAULT 1048576,
			max_app_crypto_batch_size INTEGER NOT NULL DEFAULT 256,
			require_symmetric_keys BOOLEAN NOT NULL DEFAULT TRUE,
			require_fips_keys BOOLEAN NOT NULL DEFAULT FALSE,
			min_key_size_bits INTEGER NOT NULL DEFAULT 0,
			allowed_encrypt_field_paths_json TEXT NOT NULL DEFAULT '[]',
			allowed_decrypt_field_paths_json TEXT NOT NULL DEFAULT '[]',
			denied_decrypt_field_paths_json TEXT NOT NULL DEFAULT '[]',
			block_wildcard_field_paths BOOLEAN NOT NULL DEFAULT TRUE,
			allow_deterministic_encryption BOOLEAN NOT NULL DEFAULT TRUE,
			allow_searchable_encryption BOOLEAN NOT NULL DEFAULT TRUE,
			allow_range_search BOOLEAN NOT NULL DEFAULT FALSE,
			envelope_kek_allowlist_json TEXT NOT NULL DEFAULT '[]',
			max_wrapped_dek_age_minutes INTEGER NOT NULL DEFAULT 0,
			require_rewrap_on_dek_age_exceeded BOOLEAN NOT NULL DEFAULT TRUE,
			...
			require_registered_wrapper BOOLEAN NOT NULL DEFAULT FALSE,
			local_crypto_allowed BOOLEAN NOT NULL DEFAULT FALSE,
			cache_enabled BOOLEAN NOT NULL DEFAULT FALSE,
			cache_ttl_sec INTEGER NOT NULL DEFAULT 300,
			lease_max_ops INTEGER NOT NULL DEFAULT 1000,
			max_cached_keys INTEGER NOT NULL DEFAULT 16,
			allowed_local_algorithms_json TEXT NOT NULL DEFAULT '[]',
			allowed_key_classes_for_local_export_json TEXT NOT NULL DEFAULT '[]',
			force_remote_ops_json TEXT NOT NULL DEFAULT '[]',
			require_mtls BOOLEAN NOT NULL DEFAULT FALSE,
			require_signed_nonce BOOLEAN NOT NULL DEFAULT TRUE,
			anti_replay_window_sec INTEGER NOT NULL DEFAULT 300,
		...
		`CREATE TABLE field_encryption_wrappers (
			tenant_id TEXT NOT NULL,
		...
		`CREATE TABLE field_encryption_wrapper_challenges (
			tenant_id TEXT NOT NULL,
		...
		`CREATE TABLE field_encryption_leases (
			tenant_id TEXT NOT NULL,
		...
		`CREATE TABLE field_encryption_usage_receipts (
			tenant_id TEXT NOT NULL,
