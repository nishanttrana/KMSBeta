...
var logger = log.New(os.Stdout, "[keyaccess] ", log.LstdFlags|log.Lmicroseconds)

func main() {
	cfg := pkgconfig.Load()
	if err := pkgruntimecfg.ValidateServiceConfig("kms-key-access", cfg); err != nil {
	...
	httpSrv := pkgconfig.NewHTTPServer(httpPort, pkgauditmw.Wrap(handler, publisher, "keyaccess"))
	go func() {
	...
	grpcPort := envOr("GRPC_PORT", "18270")
	...
	if reg, err := pkgconsul.NewRegistrar(cfg.ConsulAddress, "kms-key-access-"+httpPort, "kms-key-access", "127.0.0.1", mustAtoi(grpcPort)); err == nil {
...
func initNATS(url string) (*nats.Conn, nats.JetStreamContext, error) {
	nc, err := pkgevents.Connect(url, "kms-key-access", logger.Printf)
	if err != nil {
		return nil, nil, err
	}
	js, err := nc.JetStream()
	...
	_, _ = js.AddStream(&nats.StreamConfig{Name: "AUDIT_KEYACCESS", Subjects: []string{"audit.keyaccess.*"}})
	return nc, js, nil
}

func migrationPath() string {
	candidates := []string{
		filepath.Join("services", "keyaccess", "migrations"),
	...
	return filepath.Join("services", "keyaccess", "migrations")
}
...
func devMTLSConfig() (*tls.Config, error) {
	...
	tpl := &x509.Certificate{
		SerialNumber:          serial,
		Subject:               pkix.Name{CommonName: "kms-key-access-local"},
