...
func main() {
	...
	if _, err := pkgsvctls.Init(ctx, "kms-posture", pkgsvctls.Options{Logger: logger}); err != nil {
	...
	svc := NewService(
		NewSQLStore(dbConn),
		NewHTTPAuditClient(envOr("AUDIT_URL", "https://audit:8070"), 8*time.Second),
		publisher,
	)
	if governanceClient := NewHTTPGovernanceControlClient(
		envOr("GOVERNANCE_URL", "https://governance:8050"),
