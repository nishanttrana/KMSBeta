...
func main() {
	...
	httpSrv := pkgconfig.NewHTTPServer(httpPort, pkgauditmw.Wrap(handler, publisher, "qkd"))
	go func() {
...
func initNATS(url string) (*nats.Conn, nats.JetStreamContext, error) {
	nc, err := pkgevents.Connect(url, "kms-qkd", logger.Printf)
	if err != nil {
		return nil, nil, err
	}
	js, err := nc.JetStream()
...
func devMTLSConfig() (*tls.Config, error) {
	return pkgcrypto.SelfSignedMTLSConfig("kms-qkd-local")
}
