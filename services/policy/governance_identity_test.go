...
func TestGovernanceCallsCarryServiceIdentity(t *testing.T) {
	auth := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		_ = json.NewEncoder(w).Encode(map[string]string{"access_token": "kms-policy-jwt", "expires_at": "2099-01-01T00:00:00Z"})
	}))
	defer auth.Close()
	...
	for _, r := range seen {
		if r.URL.Query().Get("tenant_id") != "root" || r.Header.Get("Authorization") != "Bearer kms-policy-jwt" {
			t.Fatalf("%s %s: tenant_id=%q Authorization=%q, want root and the kms-policy service token",
