...
func isHighRiskOperation(operation string) bool {
	...
	case "key.destroy", "key.export", "key.export_policy_update", "key.approval_update", "policy.delete", "policy.update":
		return true
	default:
		return false
	}
...
func isConnectorSyncOperation(operation string) bool {
	switch strings.ToLower(strings.TrimSpace(operation)) {
	case "connector.sync", "connector.import", "byok.sync", "hyok.sync", "cloud.sync", "ekm.sync":
		return true
	default:
		return false
	}
